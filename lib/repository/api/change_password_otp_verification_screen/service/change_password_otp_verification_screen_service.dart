...
class ChangePasswordOtpVerificationScreenServices {
  Future<APIResponse> verifyOtp({required Map<String, dynamic> body}) async {
    try {
      final APIResponse response =
          await ApiHelper.postData(body: body, endPoint: "/user/verify_otp/", header: ApiHelper.getApiHeader());
      if (response.error) {
        return response;
      } else {
  ...
  Future<APIResponse> sendOtp({required Map<String, dynamic> body}) async {
    try {
      final APIResponse response =
          await ApiHelper.postData(body: body, endPoint: "/user/password/forgot/", header: ApiHelper.getApiHeader());
      if (response.error) {
        return response;
      } else {
        // LoginResModel redData = LoginResModel.fromJson(response.data["data"]);
